...
getTopRemoteUsers <- function(users=5){
  require("RPostgreSQL")
  con_t <- connect_remotepgsql()
  query <- paste("SELECT distinct(company_token)||(device_id) as user,count(*) datos
FROM locations where recorded_at  > '2023-01-01'
...
getDataByRemoteUser2 <- function(limit=100000, companytoken="fsacotoM2104K10AC-13"){
  require("RPostgreSQL")
  con_t <- connect_remotepgsql()
  query <- paste("select id,accuracy,activity_type,(company_token)||(device_id) as company_token,device_model,event,battery_level,
       is_moving,speed,heading,recorded_at,latitude,longitude,altitude from locations where recorded_at>'2023-01-01' and (company_token)||(device_id) = '",companytoken," order by recorded_at limit ",limit, sep="")
  df <- dbGetQuery(con_t, query)
  dbDisconnect(con_t)
  df <- df[order(df$recorded_at),]
  return(df)
...
changecoordsystem <- function(inputdata, longlabel="dlong", latlabel="dlat", sourceproj="+proj=longlat +datum=WGS84", targetproj="+proj=utm +zone=17 +south  ellps=WGS84"){
  library(sp)
  library(rgdal)
  data <- inputdata
